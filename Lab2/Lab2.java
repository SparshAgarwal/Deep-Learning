...
class Perceptron{
	...
	// inputs are the input for the perceptron
	// return the output of the perceptron
	// this also implements dropout and weightDecay
	public double feedForward(List<Double> inputs, double dropout, double weightDecay){
		this.inputs = inputs;
...
class NeuralNetwork{
	...
	public void train(List<List<Double>> examples, double dropout, double weightDecay){
		for(List<Double> example : examples){

			double label = example.get(0);
			predict(example.subList(1, example.size()), dropout, weightDecay); // Feedforward
	...
	public double predict(List<Double> inputs, double dropout, double weightDecay){
		List<Double> hiddenLayerOutputs = new ArrayList<Double>();

		// Forward pass for the hidden layer
		for(int i = 0; i < numHiddenUnits; i++){
			hiddenLayerOutputs.add(i, hiddenLayer.get(i).feedForward(inputs, dropout, weightDecay));
		}
	...
	public double test(List<List<Double>> examples, Boolean debug){
		...
		for(List<Double> example : examples){
			double label = example.get(0);
			double output = predict(example.subList(1, example.size()), 0, 0);
...
public class Lab2{
	...
	public static void epochExperiment(int epoch, int epochUnit, ProteinData proteinData, int numHiddenUnits, double learningRate){
		...
		for(int i = 0; i < epoch; i++){
			for(int j = 0; j < epochUnit; j++){
				nn.train(proteinData.trainList.examples, 0.2, 0.01);
			}
	...
	public static void bestAccuracy(ProteinData proteinData, double dropout, double weightDecay){
		...
		for(int i = 0; i < patience; i++){
			...
			for(int j = 0; j < epoch; j++){
				nn.train(proteinData.trainList.examples, dropout, weightDecay);
			}
